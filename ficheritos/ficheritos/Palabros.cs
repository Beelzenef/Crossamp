...
namespace PruebasFicheros
{
    class Palabros
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Mañana más, pero (no) mejor");
            Console.ReadLine();
        }
        ...
        static void Serializando()
        {
            ...
            FileStream fi = new FileStream(ruta, FileMode.Create, FileAccess.Write);
            IFormatter fmt = new BinaryFormatter();
            try {
                fmt.Serialize(fi, vid);
            }
            catch
            {
                Console.WriteLine("no puedes formatear");
            }
            finally
            {
                fi.Close();
        ...
        static void Deserializando()
        {
            string ruta = @"C:\Users\Beelzenef\Desktop\texto.txt";
            Video videito;

            FileStream fi = new FileStream(ruta, FileMode.Open, FileAccess.Read);
            IFormatter fmt = new BinaryFormatter();

            try
            {
                videito = (Video)fmt.Deserialize(fi);
            }
            catch
            {
